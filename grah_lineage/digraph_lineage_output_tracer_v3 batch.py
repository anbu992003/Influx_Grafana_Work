...
# Generator function to find all paths
...
# Prepare to write to a CSV file incrementally
output_file = "paths_output.csv"
fieldnames = [
    'path', 'levels', 'start', 'end',
    *['level' + str(i) for i in range(1, 11)],  # Arbitrary max levels
    *['bus_Elem' + str(i) for i in range(1, 11)],
    *['pde_Elem' + str(i) for i in range(1, 11)],
    *['desc' + str(i) for i in range(1, 11)],
    *['trans' + str(i) for i in range(1, 10)]  # Max transactions between levels
]
...
# Process paths and write in batches
...
print(f"\nOutput written to {output_file} in batches.")
